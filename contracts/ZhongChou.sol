...
contract ZhongChou {
     ...
     */
    function CreateNeeder(address _addr, uint _goal) public returns(uint8, uint, bool, address) {
     ...
     * 捐献money
     */
    function Contribute(uint8 _neederId) public payable {
        // 判断有没有这个众筹数据
    ...
    function TransferNeederAmount(uint8 _neederId) public payable {
        require(msg.sender == owner, "not solidity creator");
        require(neederMap[_neederId].isExists, "needer not exists");
    ...
    function GetNeederContent(uint8 _neederId) public view returns(uint, uint) {
        require(neederMap[_neederId].isExists, "needer not exists");
